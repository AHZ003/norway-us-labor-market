...
st.sidebar.markdown("---")
st.sidebar.caption("Wages are PPP-adjusted to 2023 USD. NACE J (Norway) and NAICS 51 (US) are the closest available proxies for 'tech' but are not identical.")
...
# ── Pre-compute fixed metrics (always full dataset) ───────────────────────────
...
st.markdown(
    ...
    "between Norway and the US (2010–2024), with a focus on the technology sector."
)
...
with tab1:
    ...
    st.caption("Fixed to full dataset (2010–2024) — not affected by the year filter.")

    c1, c2, c3, c4 = st.columns(4)
    if len(us_w23) and len(no_w23):
        c1.metric("Tech Wage Gap (2023)", f"${us_w23[0] - no_w23[0]:,.0f}",
                  "US pays ~40% more (PPP-adj.)")
    if len(no_u20) and len(us_u20):
        c2.metric("COVID Unemployment Gap", f"{us_u20[0] - no_u20[0]:.1f} pp",
                  "US peaked 3.5 pp higher in 2020")
    c3.metric("DiD Causal Estimate", f"{did_estimate:+.2f} pp",
              "Shock Norway absorbed vs. US")
    if len(no_emp23) and len(us_emp23):
        c4.metric("Norway Tech Share (2023)", f"{no_emp23[0]:.1f}% vs {us_emp23[0]:.1f}%",
                  "Norway has 2× relative share")
    ...
    st.caption(f"Filtered to {year_range[0]}–{year_range[1]} based on sidebar selection.")
...
with tab2:
    ...
    st.caption(f"Showing {year_range[0]}–{year_range[1]} based on sidebar filter.")

    fig_unemp = px.line(
        unemp_f, x="year", y="unemployment_rate", color="country",
        color_discrete_map=COLORS, markers=True, template=CHART_TEMPLATE,
    ...
    fig_unemp.update_layout(hovermode="x unified", yaxis_ticksuffix="%", height=CHART_H,
    ...
    # DiD section — always uses fixed 2015-2022, clearly noted
    st.subheader("Difference-in-Differences — COVID as a Natural Experiment")
    st.info(
        "This section always uses the fixed periods **2015–2019 (pre)** and **2020–2022 (post)** "
        "regardless of the year filter, since the natural experiment window is defined by COVID-19."
    )
    st.markdown(
    ...
    fig_did = px.bar(
        ...
        barmode="group", color_discrete_map=COLORS, template=CHART_TEMPLATE,
        text_auto=".2f",
        labels={"Avg Unemployment (%)": "Avg Unemployment Rate (%)"},
    )
    fig_did.update_traces(textposition="outside")
    fig_did.update_layout(yaxis_ticksuffix="%", height=CHART_H, legend_title_text="",
                          margin=dict(t=30, b=20))
    ...
    st.markdown(
        ...
        f"and a universal safety net — relative to the US."
    )
    st.caption(
        "**Parallel trends assumption:** Both countries had similar unemployment levels pre-COVID "
        f"(Norway: {pre['Norway']:.1f}%, US: {pre['United States']:.1f}%), "
        "supporting the assumption that trends would have continued similarly without the shock."
    )
...
with tab3:
    ...
    st.caption(f"Showing {year_range[0]}–{year_range[1]}. Dotted lines = linear trend (numpy.polyfit).")
    ...
    for country, color in COLORS.items():
        ...
        fig_wages.add_trace(go.Scatter(
            ...
            mode="lines+markers", name=country, line=dict(color=color, width=2.5),
            marker=dict(size=7),
        ))
    ...
    fig_wages.update_layout(
        template=CHART_TEMPLATE, hovermode="x unified", height=CHART_H,
    ...
    with col_right:
        ...
        st.caption("How much more tech pays vs. the national average.")
        ...
        fig_prem.add_hline(y=1.0, line_dash="dash", line_color="gray", line_width=1,
                           annotation_text="national avg", annotation_position="bottom right")
    ...
    fig_bar.update_layout(yaxis_tickprefix="$", yaxis_tickformat=",",
                          height=CHART_H, legend_title_text="", margin=dict(t=50, b=20))
...
with tab4:
    ...
    with col_left:
        ...
        st.caption(f"Showing {year_range[0]}–{year_range[1]}.")
        ...
        fig_emp.update_layout(hovermode="x unified", yaxis_ticksuffix="%",
                              height=CHART_H, margin=dict(t=30, b=20))
    ...
    with col_right:
        ...
        st.caption(f"Latest year in filter: {latest_year_emp}.")
        composition = emp_f[(emp_f["year"] == latest_year_emp) & (emp_f["industry"] != "Total")]

        fig_comp = px.bar(
            composition, x="employment_pct", y="industry", color="country",
            barmode="group", orientation="h", color_discrete_map=COLORS, template=CHART_TEMPLATE,
            labels={"employment_pct": "% of Workforce", "industry": "", "country": ""},
        )
        fig_comp.update_layout(xaxis_ticksuffix="%", height=CHART_H,
                               legend_title_text="", margin=dict(t=30, b=20))
...
with tab5:
    ...
    st.caption(f"Showing {year_range[0]}–{year_range[1]}. Each point is one country-year.")
    ...
    fig_corr.update_layout(height=CHART_H, margin=dict(t=30, b=20))
    ...
    st.subheader("T-test Summary — COVID Unemployment Difference")
    covid_no = unemployment[(unemployment["country"] == "Norway") & unemployment["year"].between(2020, 2022)]["unemployment_rate"]
    covid_us = unemployment[(unemployment["country"] == "United States") & unemployment["year"].between(2020, 2022)]["unemployment_rate"]
    if len(covid_no) > 1 and len(covid_us) > 1:
        t_stat, p_val = stats.ttest_ind(covid_no, covid_us)
        ...
        c3.metric("Result (α = 0.05)", "Significant" if p_val < 0.05 else "Not significant")
        st.caption(
            "Two-sample t-test comparing Norway vs. US unemployment rates during 2020–2022. "
            "Small n (3 years per group) limits statistical power — the DiD analysis above "
            "provides a more robust causal estimate."
        )

st.markdown("---")
st.caption(
    "**Built with** Python · Pandas · Plotly · Streamlit · NumPy · SciPy  |  "
    "**Limitation:** NACE J ≠ NAICS 51 exactly. Treat tech sector comparisons as indicative.  |  "
